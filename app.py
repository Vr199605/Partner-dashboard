...
def main():
    ...
    # Processamento
    df_receitas_clean = df_despesas_clean = df_seg = df_prod = df_orig = df_cli = df_cat = None
    ...
    if uploaded_file is not None:
        ...
        if len(df_receitas) > 0:
            ...
            df_receitas_clean[col_comissao] = pd.to_numeric(df_receitas_clean[col_comissao].astype(str).str.replace(',', '.').str.replace(' ', ''), errors='coerce').fillna(0)
            
            col_seguradora, col_produto, col_originador, col_cliente = df_receitas.columns[4], df_receitas.columns[10], df_receitas.columns[7], df_receitas.columns[3]
        ...
        if len(df_despesas) > 0:
            ...
            col_valor_desp, col_categoria = df_despesas.columns[4], df_despesas.columns[5]
            df_despesas_clean[col_valor_desp] = pd.to_numeric(df_despesas_clean[col_valor_desp].astype(str).str.replace(',', '.').str.replace(' ', ''), errors='coerce').fillna(0)
